...
public class MarkAreaAnswerItem implements Serializable {//Comparable<MarkAreaAnswerItem>, 
	...
	public boolean isSelectOneSelected(MarkAreaAnswer answer, float densityThreshold, int numMarkedAnswerItems) {
		return (isManualMode() && isManualSelected()) || (! isManualMode() && numMarkedAnswerItems == 1 && getDensity() < densityThreshold );
	}
	...
	public boolean equals(Object o){
		return this.itemIndex == ((MarkAreaAnswerItem)o).itemIndex;
	}
	*/

	/**
	    * Set value by user's decision
	    * filled mark: Boolean.TRUE 
	    * not filled mark: Boolean.FALSE
	    * recognize by density: null
    ...
    public int compareTo(MarkAreaAnswerItem item) {
        float c = this.density - item.density;
        if (c < 0.0f) {
            return -1;
        } else if (c > 0.0f) {
            return 1;
