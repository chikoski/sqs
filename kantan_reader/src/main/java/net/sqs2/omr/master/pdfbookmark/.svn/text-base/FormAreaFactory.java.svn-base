...
public class FormAreaFactory{

	public static FormArea create(Element gElem)throws TransformerException{
		Element pageNode = ((Element)XPathAPI.selectSingleNode(gElem, FormMasterConstants.METADATA_PAGE_ELEMENT_REPRESENTATION));
		//Element pageNode = ((Element)XPathAPI.selectSingleNode(gElem, "reader:page"));
		if(pageNode == null){
		...
		//pageNode.getAttributes().getNamedItemNS(SQSNamespaces.SQS2004READER_URI, "number").getNodeValue();

		Element xformUCElem = ((Element)XPathAPI.selectSingleNode(gElem, FormMasterConstants.RECT_METADATA_STAR_ELEMENT_REPRESENTATION)); 
		...
		area.setLabel(XPathAPI.selectSingleNode(xformUCElem, "xforms:label").getTextContent().trim().intern());
	...
	private static Rectangle getRectangle(Element gElem)throws TransformerException{
		Element rnode = (Element) XPathAPI.selectSingleNode(gElem, FormMasterConstants.RECT_ELEMENT_REPRESENTATION);
	...
	private static String[] getHints(Element gElem)throws TransformerException{
		NodeList list = XPathAPI.selectNodeList(gElem, FormMasterConstants.RECT_METADATA_STAR_HINT_ELEMENT_REPRESENTATION);
	...
	private static String getItemLabel(Element xformUCElem){
		 try {
			 return DOMUtil.getDescendantElement(xformUCElem, FormMasterConstants.LABEL_PATH).getTextContent().trim();
	...
	private static String getItemValue(Element xformUCElem){
		try {
			return DOMUtil.getDescendantElement(xformUCElem, FormMasterConstants.VALUE_PATH).getTextContent().trim();
		} catch (AbstractMethodError ex) {
		    return "";
		}
	}
